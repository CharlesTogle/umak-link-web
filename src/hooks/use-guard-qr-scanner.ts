...
import { parseGuardQrPayload } from "@/lib/parse-guard-qr-payload";
import type {
  GuardQrPayload,
  GuardQrScannerPhase,
  GuardQrScannerState,
...
const CAMERA_UNSUPPORTED_MESSAGE =
  "Camera scanning is not available in this browser. Use manual entry below.";
...
export function useGuardQrScanner({ onDetected }: UseGuardQrScannerOptions) {
    ...
    try {
      ...
      const detectQrCode = async () => {
        ...
        try {
          ...
          let payload: GuardQrPayload;

          try {
            payload = parseGuardQrPayload(rawValue);
          } catch {
            scheduleNextScan(detectQrCode);
            return;
          }
  ...
  return {
    ...
    isSupported: supportsGuardCameraScan(),
    openCamera,
    state,
