...
import { guardPrimaryRoutes } from "@/app/guard/routes/guard-routes";
...
export function GuardSidebar() {
  ...
  const profileName = user?.user_name?.trim() || "Guard User";
  ...
  return (
    <aside className="m-4 flex h-[calc(100vh-2rem)] w-[calc(100vw-2rem)] max-w-80 shrink-0 lg:w-80">
      <div className="flex w-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <h2 className="mb-4 text-lg font-semibold text-slate-900">Guard Portal</h2>

        <nav className="space-y-2 overflow-auto pr-1">
          {guardPrimaryRoutes.map((route) => {
