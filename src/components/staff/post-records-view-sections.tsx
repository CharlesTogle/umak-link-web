...
export function PostRecordsFeed(props: {
  ...
  records: PostRecord[];
  hasNextPage?: boolean;
  onAction: (action: any, record: PostRecord) => void;
}) {
  return (
    <div ref={props.feedRef} onScroll={props.onScroll} className="min-h-0 space-y-4 overflow-y-auto pr-1 lg:col-span-8">
      ...
      {props.records.length === 0 && !props.isLoading ? (
        ...
        <div className="space-y-4">
          {props.records.map((record) => <PostRecordCard key={record.postId} record={record} onAction={props.onAction} />)}
        </div>
      )}
      {!props.hasNextPage && props.records.length > 0 ? <p className="text-center text-sm text-slate-500">You&apos;re all caught up!</p> : null}
    </div>
  );
}

export function PostRecordsSidebar(props: {
  ...
  return (
    <aside className="min-h-0 space-y-3 overflow-y-auto pr-1 lg:col-span-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <button type="button" onClick={props.onSearch} className="flex w-full items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-500 shadow-sm hover:border-slate-300">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <button type="button" onClick={props.onRefresh} className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600 shadow-sm hover:bg-slate-50">
          ...
          {props.isRefetching ? "Refreshing..." : "Refresh records"}
        </button>
      </div>
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <select id="post-record-sort" value={props.sortDir} onChange={(event) => props.onSortChange(event.target.value as PostRecordSortDirection)} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700">
            ...
            <section key={group.key} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="flex flex-wrap gap-2">
                {group.options.map((option) => {
                  const isActive = props.filters[group.key] === option.value;
                  return (
                    <button key={`${group.key}-${option.value}`} type="button" onClick={() => props.onFilterChange(group.key, option.value)} className={`rounded-full border px-3 py-1.5 text-sm transition ${isActive ? "border-[#1D2981]/20 bg-[#1D2981]/10 font-medium text-[#1D2981]" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-100"}`}>
                      {option.label}
                    </button>
                  );
                })}
...
export function PostRecordsNotifyModal(props: { isOpen: boolean; onCancel: () => void; onConfirm: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-lg">
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          <button type="button" onClick={props.onCancel} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>
          <button type="button" onClick={props.onConfirm} className="rounded-full bg-[#1D2981] px-4 py-2 text-sm font-medium text-white hover:bg-[#16206a]">Confirm</button>
        </div>
      </div>
    </div>
  );
